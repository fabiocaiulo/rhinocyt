...
export class ModelService {
  ...
  // private modelsUrl = 'https://rhinocyt.herokuapp.com/api/models/';

  // URL to Local Web Server
  private modelsUrl = 'http://localhost:8080/api/models/';
  ...
  saveModel(name: string, dataset: any): Observable<Response> {
    const url = this.modelsUrl + 'save?name=' + name;
  ...
  loadModel(name: string): Observable<Model> {
    const url = this.modelsUrl + 'load/' + name;
